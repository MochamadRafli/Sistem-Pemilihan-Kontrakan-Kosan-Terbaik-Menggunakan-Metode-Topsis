...
if(empty($foto)){ // Jika user tidak memilih file foto pada form
  ...
  $sql = $pdo->prepare("UPDATE tab_detail SET nama Kos=:nama Kos, telpon=:telpon, alamat=:alamat WHERE id=:id");
  $sql->bindParam(':nama Kos', $nama_Kos);
  ...
  if($sql){ // Cek jika proses simpan ke database sukses atau tidak
    // Jika Sukses, Lakukan :
    header("location: editalternatif2.php"); // Redirect ke halaman index.php
  }else{
    // Jika Gagal, Lakukan :
  ...
  if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
    ...
    $sql = $pdo->prepare("SELECT foto FROM tab_detail WHERE id=:id");
    $sql->bindParam(':id', $id);
    ...
    // Proses ubah data ke Database
    $sql = $pdo->prepare("UPDATE tab_detail SET nama Kos=:nama Kos, telpon=:telpon, alamat=:alamat, foto=:foto WHERE id=:id");
    $sql->bindParam(':nama Kos', $nama_Kos);
    $sql->bindParam(':telpon', $telpon);
    $sql->bindParam(':alamat', $alamat);
